...
public class a_Projectile : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D other)
    {
        ...
        {
            other.gameObject.GetComponent<Rigidbody2D>().AddForce(Vector3.Normalize(other.gameObject.transform.position - gameObject.transform.position) * f_speed, ForceMode2D.Impulse);
            Destroy(gameObject);
        }
